...
const ProgressOverview: React.FC<ProgressOverviewProps> = ({ progress }) => {
  ...
  const highestLevelWithProgress = progress
    .filter(p => p.completedProblems > 0)
    ...
    <div className="card p-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-4">
          ...
          <div className="flex items-end">
            <span className="text-3xl font-bold text-primary-600 dark:text-primary-400">{overallProgress}%</span>
          </div>
        ...
        <div className="bg-secondary-50 dark:bg-secondary-900/20 rounded-lg p-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">Problems Completed</p>
          <div className="flex items-end">
            <span className="text-3xl font-bold text-secondary-600 dark:text-secondary-400">{totalProblems}</span>
            <span className="ml-1 text-sm text-gray-500 dark:text-gray-400">problems</span>
          </div>
          <p className="mt-1 text-sm">
            <span className="font-medium text-secondary-600 dark:text-secondary-400">{totalCorrect}</span> correct answers
          </p>
        </div>
        ...
        <div className="bg-accent-50 dark:bg-accent-900/20 rounded-lg p-4">
          ...
          <div className="flex items-end">
            <span className="text-3xl font-bold text-accent-600 dark:text-accent-400">{overallAccuracy}%</span>
          </div>
          <div className="mt-2 h-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
            <div 
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 mb-4">
        <div className="flex justify-between items-center">
          <div>
            <h4 className="font-bold text-xl">Level {highestLevelWithProgress}</h4>
          ...
          <div className="flex items-center">
            ...
            <span className="text-lg font-semibold">
              {progress.find(p => p.level === highestLevelWithProgress)?.completedProblems || 0} problems solved
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        {totalProblems > 0 ? (
          <p className="text-gray-600 dark:text-gray-400">
            You've completed {totalProblems} challenges with {overallAccuracy}% accuracy. Keep going!
          </p>
        ) : (
          <p className="text-gray-600 dark:text-gray-400">
            You haven't solved any problems yet. Start learning now!
          </p>
        )}
      </div>
    </div>
